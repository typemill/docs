...
You can also open link in a [new tab](https://typemill.net){target="_blank"}
...
You can format short code snippets inside a sentence by wrapping them with a single backtick.
...
For larger code examples, use three backticks before and after the code block.
...
Special Tipp: To display backticks inside a code block without them being interpreted, simply indent the entire code block with four spaces. This is especially helpful for code documentations.
...
You¹ can² copy and pasteⁿ charactersₓ from [unicodeⁿ tables͓](https://unicode-table.com) if̥ you @ need ® some special characters.

## HTML

Typemill does not allow any HTML tags in Markdown for security reasons, but you can use the [HTML plugin](https://plugins.typemill.net/html) for tags like `<div>`, `<span>`, `<sub>`, `<sup>`, `<address>`, and more. 
